...
@if (isset($action_plans))
	...
	<h4><b>Planes de acci&oacute;n creados para {{ $org }}</b></h4>
		...
		@endforeach
				...
				@foreach ($action_plan as $action_plan)
					<tr>
				...
				@foreach (Session::get('roles') as $role)
					@if ($role != 6)
						...
						<td>
						<button class="btn btn-danger" onclick="eliminar({{ $action_plan['id'] }},'{{ $action_plan['name'] }}','action_plan','El plan de acción')">Eliminar</button></td>
					<?php break; ?>
					@endif
				@endforeach
					</tr>
