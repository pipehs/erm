		...
		if (org)
		{
			...
			if ($('#type2_test_'+test).val() == 1)
			{
				...
				$.get('get_controls.'+org, function (result) {
					...
					select += '<label for="control_id_test'+test+'" class="col-sm-4 control-label">Select control</label>';
					select += '<div class="col-sm-4">'
					select += '<select name="control_id_test_'+test+'" class="form-control" required="true">';
					select += '<option value="" selected disabled>- Select -</option>';
			...
			else if ($("#type2_test_"+test).val() == 2)
			{
				...
				$.get('get_risks.'+org, function (result) {
					...
					var select = '<div class="form-group">';
					select += '<label for="control_id_test'+test+'" class="col-sm-4 control-label">Select risk</label>';
					select += '<div class="col-sm-4">'
			...
			else if ($("#type2_test_"+test).val() == 3)
			{
				$.get('get_subprocesses.'+org, function (result) {
					...
					var select = '<div class="form-group">';
					select += '<label for="control_id_test'+test+'" class="col-sm-4 control-label">Select subprocess</label>';
					select += '<div class="col-sm-4">'
